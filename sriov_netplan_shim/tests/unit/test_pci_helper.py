...
from sriov_netplan_shim.tests.unit import pci_responses
from sriov_netplan_shim.tests.unit.test_utils import patch_open


def check_device(device, attr_dict):
    equal = (
        device.interface_name == attr_dict["interface_name"]
        and device.mac_address == attr_dict["mac_address"]
        and device.pci_address == attr_dict["pci_address"]
        and device.state == attr_dict["state"]
    )
    return equal
